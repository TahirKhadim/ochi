...
import './Nav.css'; // Import the custom CSS for blinking effect
...
const Nav = () => {
    ...
    useGSAP(() => {
        if (open) {
            gsap.to('#sb-links', {
                ...
                ease: 'back.inOut',
                opacity: 1,
                stagger: 0.3,  // Delays each item by 0.3 seconds for stagger effect
                duration: 1,
            });
        }
    ...
    return (
        <div className='flex items-center justify-between px-10 py-8 bg-zinc-800'>
            ...
            <div className='flex items-center gap-10'>
                {links.map((item, i) => (
                    <li key={i} id='f-menu' className={`hidden opacity-0 md:flex list-none font-semibold text-sm ${item.name === "Contact Us" ? 'lg:ml-44' : ''}`}>
                        <Link to={item.link} className='nav-link'>{item.name}</Link>
                    </li>
                ))}
            </div>

            <div className='text-3xl md:hidden duration-75 text-white'>
                {open ? <RxCross2 onClick={handleCross} /> : <CiMenuFries onClick={handleOpen} />}
            </div>

            <div id='menu' className={`fixed  md:hidden top-16 left-0 w-full h-screen bg-black text-white transition-transform duration-500 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
                <ul className='flex flex-col items-center justify-center py-6 relative'>
                    {links.map((item, i) => (
                        <li key={i} id='sb-links' className='my-4 opacity-0'>
                            <Link to={item.link} className='nav-link'>{item.name}</Link>
                        </li>
                    ))}
                </ul>
